...
static u8 aecp_aem_get_counters_response(struct entity *entity, struct aecp_aem_pdu *aecp_rsp, u16 *len, u16 descriptor_type, u16 descriptor_index)
{
	...
	*len += sizeof(struct aecp_aem_get_counters_rsp_pdu);
	...
	case AEM_DESC_TYPE_STREAM_INPUT:
	{
		...
		get_counters_rsp->counters_block[AECP_AEM_COUNTER_STREAM_INPUT_STREAM_RESET] = htonl(stream_input_dynamic->diagnostic_counters.stream_interrupted);
	...
	case AEM_DESC_TYPE_STREAM_OUTPUT:
	{
		...
		get_counters_rsp->counters_block[AECP_AEM_COUNTER_STREAM_OUTPUT_MEDIA_RESET] = htonl(stream_output_dynamic->diagnostic_counters.media_reset);
		get_counters_rsp->counters_block[AECP_AEM_COUNTER_STREAM_OUTPUT_TIMESTAMP_UNCERTAIN] = htonl(stream_output_dynamic->diagnostic_counters.timestamp_uncertain);
		get_counters_rsp->counters_block[AECP_AEM_COUNTER_STREAM_OUTPUT_FRAMES_TX] = htonl(stream_output_dynamic->diagnostic_counters.frame_tx);
	}
	break;
	}
